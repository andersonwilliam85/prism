...
class ConfigMerger:
    ...
    def _merge_value(self, key: str, base_value: Any, overlay_value: Any, level: str) -> Any:
        ...
        # Both are dicts - deep merge by default
        if isinstance(base_value, dict) and isinstance(overlay_value, dict):
            explicit_strategies = self.rules.get("merge_strategy", {}).get("objects", {})
            if strategy == "user_only" and level != "user":
                ...
                return deepcopy(overlay_value)  # Only override dicts when explicitly configured
            else:  # deep_merge is the default for dicts
...
def load_merged_config(
    ...
    return merger.load_merged_config(company=company, sub_org=sub_org, department=department, team=team, user=user)
